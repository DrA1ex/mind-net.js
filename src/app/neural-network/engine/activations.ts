...
class SigmoidActivation implements IActivation {
    ...
    moment(m: matrix.Matrix1D): matrix.ManagedMatrix1D {
        ...
        const result = matrix.matrix1d_unary_op(sigm, x => x * (1 - x));
        sigm.free();
...
class LeakyReluActivation implements IActivation {
    value(m: matrix.Matrix1D, alpha = 0.1): matrix.ManagedMatrix1D {
        return matrix.matrix1d_unary_op(m, x => x > 0 ? x : x * alpha);
    }

    moment(m: matrix.Matrix1D, alpha = 0.1): matrix.ManagedMatrix1D {
